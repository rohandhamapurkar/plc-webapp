		...
		>
			<template v-slot:append>
				<v-badge dot overlap :value="areFiltersApplied">
					<v-icon color="secondaryFontColor" @click.stop="toggleAdvanceSearchSection()">{{ appendIcon }}</v-icon>
				</v-badge>
			</template>
			...
			<div v-show="menu">
				<v-card class="advance-search-card" outlined elevation="0">
					<div class="filters-container">
						<template v-for="(filter, filterIndex) in filterConfig">
							...
							<div
								...
								:class="checkForClass(filter.classes)"
							>
								<v-switch v-model="filterObject[filter.key]" :label="filter.name" class="form-item" inset></v-switch>
							</div>
							<div
							...
							>
								...
								>
									...
									<v-date-picker v-model="filterObject[filter.key]" @input="pickerMenu = false">
										<v-btn text color="primary" @click="clearDate(filter.key)"> Clear </v-btn></v-date-picker
									>
								</v-menu>
							</div>
						</template>
					...
					<v-card-actions>
						...
						<v-btn :disabled="!areFiltersApplied" color="error" text @click="clearFilters()"> Clear </v-btn>
...
export default {
	...
	methods: {
		...
		performAdvanceSearch() {
			...
			this.toggleAdvanceSearchSection();
			if (this.queryString != "") this.filterObject.searchText = this.queryString;
